<script>
    ...
    async function getUser(email, password) {
        const response = await fetch("http://localhost:8830/events/login?email=" + email + "&password=" + password);
        ...
        if (user.error == 0) {
            ...
            const response2 = await fetch("http://localhost:8830/events/isProfessional?userID=" + $idGlobal);
            ...
            if ($medico == "true") {
                ...
                const response3 = await fetch("http://localhost:8830/events/isClient?userID=" + $idGlobal);
